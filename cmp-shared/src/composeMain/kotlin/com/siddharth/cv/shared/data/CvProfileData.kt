...
/**
 * Transcribed verbatim from cv-siddharth/src/data/profile.ts.
 *
 * Every metric string in this file is an audited claim in the source repo.
 * Changing "~87%", "50k+", "50% → 95%" or "80%" here is a claim-audit gate
 * failure, not a typo — diff against profile.ts before editing anything.
 */
...
val metrics = listOf(
    ...
    Metric("95%", "GPS accuracy", "up from 50% — predictive dead reckoning"),
    Metric("80%", "crash reduction", "Crashlytics + structured concurrency fixes"),
    Metric("~87%", "UI-layer Compose", "455k of 523k UI LOC in a ~960k LOC app; per-screen parity verification"),
)
...
val caseStudies = listOf(
    ...
    CaseStudy(
        ...
        problem = """A fast-growing ~960k-LOC app had a crash rate hurting its Play Store rating, driven by untraceable threading bugs.""",
        approach = listOf(
...
/** The site's own interactive surfaces — `to` is a web route on the React site. */
