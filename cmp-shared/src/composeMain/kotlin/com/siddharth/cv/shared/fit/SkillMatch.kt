 ...
 */
val OFF_STACK_TERMS: Map<String, String> =
    linkedMapOf(
        "rust" to "Rust",
        "golang" to "Go",
...
private val REQUIREMENT_HEADING =
    Regex(
...
/** "Only the first few lines", as a number. */
private const val VerdictLinesScanned = 6
...
fun matchJd(text: String): SkillMatchResult {
    ...
    fun weigh(
        preferred: Boolean,
        depth: Depth? = null,
    ): Double = (if (preferred) PREFERRED_WEIGHT else 1.0) * (depth?.let { DEPTH_WEIGHT.getValue(it) } ?: 1.0)
...
@Serializable
private data class JdFitWireGap(
    val need: String? = null,
...
private fun JdFitWire.toReport(): JdFitReport? {
    ...
    val strengthRows =
        strengths.take(MAX_ROWS).mapNotNull { s ->
    ...
    val gapRows =
        gaps.take(MAX_ROWS).mapNotNull { g ->
