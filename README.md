...
|---|---|
| AI chat, generative UI | The React panel renders `[[rooms]]`-style directives as real components; this port has no widget renderer, so whole-line directives are **stripped** before display (`stripDirectives`, self-checked). The reply reads cleanly, it just doesn't gain the component. Slash commands and Web Speech voice I/O are also absent; each is a subsystem, not a toggle. The JD fit analyzer shipped separately, as its own self-contained card rather than through this transcript: see the **Fit check** row below. |
